...
class PhysicsObject:
    ...
    def __init__(self, pos, mass, size, is_static=False, shape=Shape.BOX, material=Material.STONE):
        ...
        # A. Numerical Stability: Use numpy arrays for vectorization benefits.
        ...
        self.possible_spins = np.zeros(NUM_QUANTUM_STATES, dtype=np.float64)
    ...
    def calculate_inertia_tensor(self):
        """Calculates the inverse inertia tensor for the object."""
        if self.is_static:
            return
        # Simplified for a box shape
    ...
    def initialize_quantum_state(self):
        ...
        for i in range(NUM_QUANTUM_STATES):
            # A. Numerical Stability: Use Gaussian distribution for initialization.
            self.possible_positions[i] = Vector3(
                self.pos.x + np.random.normal(0, 0.1),
                self.pos.y + np.random.normal(0, 0.1),
                self.pos.z + np.random.normal(0, 0.1)
            )
...
class PhysicsState:
    ...
    def add_object(self, pos, mass, size, is_static=False, shape=Shape.BOX, material=Material.STONE):
        ...
        if len(self.objects) < MAX_OBJECTS:
            ...
            if obj.is_quantum:
                obj.wave_amplitude = (PLANCK_CONSTANT * self.global_planck_factor) / (obj.mass * math.sqrt(speed_sq) + 1e-10)
    ...
    def update(self, dt):
        ...
        # B. Entropy & ACE: Call uncertainty function to enable feedback loop.
    ...
    def integrate(self, dt):
        ...
        for obj in self.objects:
            ...
            # Angular motion
            torque_vec = np.array([obj.torque.x, obj.torque.y, obj.torque.z])
            ang_accel = obj.inertia_tensor_inv @ torque_vec
            obj.ang_vel.x += ang_accel[0] * dt
            obj.ang_vel.y += ang_accel[1] * dt
            obj.ang_vel.z += ang_accel[2] * dt
            obj.torque = Vector3()
            ...
            if obj.is_quantum and not obj.is_observed:
                ...
                exp_pos = Vector3()
                exp_vel = Vector3()
                # Use squared amplitudes for expectation values
                probs = obj.amplitudes**2
                for i in range(NUM_QUANTUM_STATES):
                    exp_pos.x += obj.possible_positions[i].x * probs[i]
                    exp_pos.y += obj.possible_positions[i].y * probs[i]
                    exp_pos.z += obj.possible_positions[i].z * probs[i]
                    exp_vel.x += obj.possible_velocities[i].x * probs[i]
                    exp_vel.y += obj.possible_velocities[i].y * probs[i]
                    exp_vel.z += obj.possible_velocities[i].z * probs[i]
                obj.pos = exp_pos
                obj.vel = exp_vel
            else:
            ...
            # C. Rotational Dynamics: Update orientation from angular velocity
            w, x, y, z = obj.orientation.w, obj.orientation.x, obj.orientation.y, obj.orientation.z
            avx, avy, avz = obj.ang_vel.x, obj.ang_vel.y, obj.ang_vel.z

            w_dot = -0.5 * (avx * x + avy * y + avz * z)
            x_dot =  0.5 * (avx * w + avz * y - avy * z)
            y_dot =  0.5 * (avy * w - avz * x + avx * z)
            z_dot =  0.5 * (avz * w + avy * x - avx * y)

            obj.orientation.w += w_dot * dt
            obj.orientation.x += x_dot * dt
            obj.orientation.y += y_dot * dt
            obj.orientation.z += z_dot * dt
            obj.orientation.normalize()

            # C. Rotational Dynamics: Toy "quantized gyroscope"
            if obj.is_quantum:
                spin_expectation = np.sum(obj.amplitudes**2 * obj.possible_spins)
                obj.ang_vel.y += (spin_expectation - 0.5) * 0.01
    ...
    def normalize_amplitudes(self, obj):
        ...
        sum_sq = np.sum(obj.amplitudes**2)
        if sum_sq > 1e-10:
                ...
                for j in range(i + 1, len(self.objects)):
                    ...
                    if other.material_type == Material.QUANTUM and other.entangled_partner_idx == -1:
                        ...
                        if dist_sq < self.quantum_entanglement_range**2:
                            ...
                            obj.possible_spins = 1.0 - other.possible_spins
                            print(f"Objects {i} and {j} are now entangled!")
                            break

    def resolve_collisions(self):
        """Detects and resolves collisions between objects, including penetration and impulse."""
        for i, obj1 in enumerate(self.objects):
            for j in range(i + 1, len(self.objects)):
                obj2 = self.objects[j]
                ...
                if (abs(obj1.pos.x - obj2.pos.x) * 2 < (obj1.width + obj2.width) and
                    ...
                    if min_overlap == overlap_x: normal = Vector3(np.sign(obj1.pos.x - obj2.pos.x), 0, 0)
                    elif min_overlap == overlap_y: normal = Vector3(0, np.sign(obj1.pos.y - obj2.pos.y), 0)
                    else: normal = Vector3(0, 0, np.sign(obj1.pos.z - obj2.pos.z))

                    total_inv_mass = obj1.inv_mass + obj2.inv_mass
                    if total_inv_mass > 0:
                        correction = max(min_overlap - 0.01, 0.0) / total_inv_mass * 0.8
                    ...
                    # --- Impulse Resolution ---
                    rel_vel = Vector3(obj2.vel.x - obj1.vel.x, obj2.vel.y - obj1.vel.y, obj2.vel.z - obj1.vel.z)
                    vel_along_normal = rel_vel.x * normal.x + rel_vel.y * normal.y + rel_vel.z * normal.z
                    ...
                    restitution -= self.emotional_field[EmotionalState.FEAR] * 0.5
                    restitution = np.clip(restitution, 0, 1)

                    impulse_scalar = -(1 + restitution) * vel_along_normal / total_inv_mass

                    obj1.vel.x -= impulse_scalar * normal.x * obj1.inv_mass
                    obj1.vel.y -= impulse_scalar * normal.y * obj1.inv_mass
                    obj1.vel.z -= impulse_scalar * normal.z * obj1.inv_mass
                    obj2.vel.x += impulse_scalar * normal.x * obj2.inv_mass
                    obj2.vel.y += impulse_scalar * normal.y * obj2.inv_mass
                    obj2.vel.z += impulse_scalar * normal.z * obj2.inv_mass
    ...
    def apply_damping(self):
        ...
        for obj in self.objects:
            ...
            obj.vel.y *= DAMPING
            obj.vel.z *= DAMPING
            obj.ang_vel.x *= DAMPING
            obj.ang_vel.y *= DAMPING
            obj.ang_vel.z *= DAMPING
...
if __name__ == '__main__':
    # E. Visualization: The following code can be hooked to a 3D renderer.
    # For example, one could render obj.pos for classical objects,
    # and a transparent cloud of obj.possible_positions for quantum objects.
    ...
    for step in range(500):
        ...
        if step > 0 and step % 100 == 0:
            # B. Emotional Feedback: Periodically apply an emotional state
            physics_world.emotional_field.fill(0.0)
            physics_world.emotional_field[EmotionalState.FEAR] = 0.6
            print(">>> A wave of FEAR washes over the simulation! <<<")
        ...
        for i, obj in enumerate(physics_world.objects):
            if not obj.is_static:
                print(f"Object {i}: Pos={obj.pos}, Vel={obj.vel}, AngVel={obj.ang_vel}, Quantum={obj.is_quantum}, Observed={obj.is_observed}")
